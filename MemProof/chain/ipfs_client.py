...
def publish_to_ipfs(bundle: dict) -> str | None:
    """
    Upload a proof bundle dict to IPFS via Pinata.
    Returns the IPFS CID on success, None on failure.

    The CID is content-addressed: changing one byte changes the CID.
    """
    ...
    src_hash = bundle.get('source_hash', 'unknown')[:12]
    payload = json.dumps({
        'pinataContent': bundle,
        'pinataMetadata': {
            'name': f'memproof-{src_hash}',
        },
    }).encode('utf-8')
