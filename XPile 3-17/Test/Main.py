...
sys.path.insert(0, os.path.join(_base, "AST"))
sys.path.insert(0, os.path.join(_base, "Semantic"))
sys.path.insert(0, os.path.join(_base, "Codegen"))
sys.path.insert(0, os.path.join(_base, "Compiler"))
sys.path.insert(0, os.path.join(_base, "REPL"))
...
def load_codegen(target):
    if target == "c":
        ...
        return generate, ".c"
    if target == "llvm":
        from codegen_llvm import generate
        return generate, ".ll"
    if target == "wasm":
        from codegen_llvm import generate
    ...
    if target == "python":
        ...
        return generate, ".py"
    if target == "js":
        from codegen_js import generate
        return generate, ".js"
    if target == "bytecode":
        from codegen_bytecode import generate
        return generate, ".ulb"
...
def compile_file(path, target, out_path=None, norm_level=0, profile=False, quiet=False):
    ...
    if norm_level > 0:
        ...
        from normalize import detect_norm_level, normalize as norm
...
def _compile_native(program, path, filename, out_path, profile):
    ...
    if llc and gcc:
        ...
        from codegen_llvm import generate as gen_llvm
    ...
    if gcc:
        ...
        from codegen_c import generate as gen_c
    ...
    if vm_mode:
        ...
        for candidate in [
            ...
            if os.path.isfile(candidate):
                ...
                break
        if ulvm is None:
            print("error: ulvm executable not found in VM/ directory", file=sys.stderr)
            sys.exit(1)
        # Run through VM
