...
class C2UL(c_ast.NodeVisitor):
    ...
    def _hoist_assignments_from_cond(self, cond_node):
        ...
        and compound assignments: while (sz >>= 1) {...}
        ...
        replaced by its LHS.
        """
        self._hoisted_cond_assignments = []  # track for while-loop body repetition

        def _hoist(node):
            if node is None:
                return node
            if isinstance(node, c_ast.Assignment):
                # Handle all assignment ops: =, +=, -=, >>=, <<=, &=, |=, ^=, etc.
                self._visit_assignment(node)
                self._hoisted_cond_assignments.append(node)
                return node.lvalue
            if isinstance(node, c_ast.BinaryOp):
    ...
    def _visit_while(self, node):
        ...
        if body_has_break:
            ...
            cond = self._hoist_assignments_from_cond(node.cond)
            hoisted = getattr(self, '_hoisted_cond_assignments', [])
            ...
            if node.stmt:
                if isinstance(node.stmt, c_ast.Compound):
                    ...
                    self._visit_stmt(node.stmt)
            # Repeat hoisted assignments at end of body
            for assign_node in hoisted:
                self._visit_assignment(assign_node)
            ...
            if node.stmt:
                if isinstance(node.stmt, c_ast.Compound):
                    ...
                    self._visit_stmt(node.stmt)
            # Repeat hoisted assignments at end of body so condition updates each iteration
            for assign_node in hoisted:
                self._visit_assignment(assign_node)
            self.depth -= 1
            self.emit("end while")
...
def _gcc_preprocess(source_path, extra_defines=None):
    ...
    """
    gcc_candidates = ["gcc", "/c/Users/azt12/mingw64/bin/gcc", "cc"]
    gcc = None
