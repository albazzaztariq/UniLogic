...
footer_t* get_foot(node_t* node)
{
    return ((footer_t)((((char)node) + sizeof(node_t)) + node->size_c));
}
...
int main(void)
{
    ...
    snprintf(_cast_buf_0, sizeof(_cast_buf_0), "%d", (f->header == n));
    printf("%s\n", __ul_strcat("foot->header == n: ", _cast_buf_0));
    uint64_t expected_offset = (sizeof(node_t) + 64);
    uint64_t actual_offset = ((uint64_t)(((char)f) - buf));
    static char _cast_buf_1[64];
    snprintf(_cast_buf_1, sizeof(_cast_buf_1), "%u", expected_offset);
    printf("%s\n", __ul_strcat("expected_offset=%zu actual_offset=%zu match=", _cast_buf_1));
    node_t* recovered = f->header;
    static char _cast_buf_2[64];
    snprintf(_cast_buf_2, sizeof(_cast_buf_2), "%u", recovered->size_c);
    static char _cast_buf_3[64];
    snprintf(_cast_buf_3, sizeof(_cast_buf_3), "%u", recovered->hole);
    printf("%s\n", __ul_strcat(__ul_strcat(__ul_strcat("recovered->size=", _cast_buf_2), " recovered->hole="), _cast_buf_3));
    uint32_t x = 42;
    uint32_t* px = (&x);
    static char _cast_buf_4[64];
    snprintf(_cast_buf_4, sizeof(_cast_buf_4), "%u", x);
    static char _cast_buf_5[64];
    snprintf(_cast_buf_5, sizeof(_cast_buf_5), "%d", (*px));
    printf("%s\n", __ul_strcat(__ul_strcat(__ul_strcat("x=", _cast_buf_4), " *px="), _cast_buf_5));
    (*px) = 99;
    static char _cast_buf_6[64];
    snprintf(_cast_buf_6, sizeof(_cast_buf_6), "%u", x);
    printf("%s\n", __ul_strcat("after *px=99: x=", _cast_buf_6));
    static char _cast_buf_7[64];
    snprintf(_cast_buf_7, sizeof(_cast_buf_7), "%d", (px == (&x)));
    printf("%s\n", __ul_strcat("px == &x: ", _cast_buf_7));
    return 0;
}
