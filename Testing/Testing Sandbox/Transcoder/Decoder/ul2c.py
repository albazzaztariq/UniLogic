...
class CGen:
    ...
    def _const_value(self, node):
        ...
        if t == Identifier:
            return node.name
        raise ValueError(f"codegen_c: cannot evaluate constant expression of type {t.__name__}")
    ...
    def _resolve_field_type(self, node):
        ...
        if isinstance(node.target, Identifier):
            ...
            # 'self' inside an object method — resolve against current object type
            if node.target.name == "self" and self._current_object:
                for f in self.all_fields(self._current_object.name):
    ...
    _UNSIGNED_TYPES = {"uint8", "uint16", "uint32", "uint64"}

    def _fmt_for_type_name(self, tname):
        """Return printf format specifier for a UL type name."""
        if tname in ("float", "double"):  return "%f"
        if tname == "string":             return "%s"
        if tname in self._UNSIGNED_TYPES: return "%u"
        return "%d"
    ...
    def fmt_specifier(self, node):
        ...
        if t == Cast:
            ...
            return self._fmt_for_type_name(node.target_type.name)
        if t == Identifier:
            vtype = self.var_types.get(node.name)
            if vtype:
                return self._fmt_for_type_name(vtype.name)
            return "%d"
        if t == Call:
            fn_decl = self._func_decls.get(node.name)
            if fn_decl and fn_decl.return_type:
                # Pointer to char = string = %s
                if fn_decl.return_type.pointer and fn_decl.return_type.name in ("char", "string"):
                    return "%s"
                if fn_decl.return_type.pointer:
                    return "%p"
                return self._fmt_for_type_name(fn_decl.return_type.name)
            return "%d"
        if t == BinaryOp:
        ...
        if t == FieldAccess:
            ...
            if ftype:
                return self._fmt_for_type_name(ftype.name)
            return "%d"
        if t == Index:
            ...
            if isinstance(node.target, Identifier):
                ...
                if vtype:
                    ...
                    if hasattr(vtype, 'name'): return self._fmt_for_type_name(vtype.name)
                    if hasattr(vtype, 'element_type'):
                        ...
                        return self._fmt_for_type_name(ename)
            return "%d"
        if t == SliceExpr:
            ...
            return "%s"
        if t == MethodCall:
            # Look up method return type from object definition (including inherited methods)
            ...
            if target_type and target_type.name in self.type_decls:
                ...
                while type_name:
                    obj_decl = self.type_decls.get(type_name)
                    if obj_decl and isinstance(obj_decl, ObjectDecl):
                        for m in obj_decl.methods:
                            if m.name == node.method and m.return_type:
                                return self._fmt_for_type_name(m.return_type.name)
                        type_name = obj_decl.parent
                    else:
                        break
            return "%d"
        return "%d"
    ...
    def gen_program(self, program):
        ...
        self._python_compat = self._dr.get("python_compat") == "true"
        if dr_directives:
        ...
        # str_char_at always emitted — needed whenever string indexing s[i] is used
        ...
        self.emit("static char* __ul_char_from_code(int code) {")
        self.emit("    char* r = (char*)__ul_malloc(2); r[0] = (char)code; r[1] = 0; return r; }")
        ...
        # Float print helper: matches Python's float repr (e.g. 3.14 not 3.140000, 10.0 not 10)
        self.emit("static void __ul_print_float(double v) {")
        self.emit("    char _buf[64]; snprintf(_buf, sizeof(_buf), \"%.10g\", v);")
        self.emit("    int _has_dot = 0; for (int _i = 0; _buf[_i]; _i++) if (_buf[_i] == '.' || _buf[_i] == 'e') { _has_dot = 1; break; }")
        self.emit("    if (!_has_dot) { int _l = (int)strlen(_buf); _buf[_l] = '.'; _buf[_l+1] = '0'; _buf[_l+2] = 0; }")
        self.emit("    printf(\"%s\\n\", _buf); }")
        if self._profile:
        ...
        for decl in program.decls:
            if isinstance(decl, EnumDecl):
                ...
                self.emit(f"typedef enum {decl.name} {decl.name};")
                self.emit("")
        ...
        for decl in program.decls:
            if isinstance(decl, VarDecl):
                ...
                if decl.type_.is_array and isinstance(decl.init, ArrayLiteral):
                    ...
                    val = self.gen_expr(decl.init)
                    # User-defined struct types cannot be initialized with a bare 0.
                    # Emit {0} aggregate initializer instead so gcc accepts it.
                    if (decl.type_.name in self.type_decls and val == "0"
                            and not decl.type_.is_array and not decl.type_.is_list):
                        self.emit(f"{ctype} {decl.name} = {{0}};")
                    else:
                        self.emit(f"{ctype} {decl.name} = {val};")
                else:
        ...
        for decl in program.decls:
            ...
            # Pre-populate _nested_name_map with ALL sibling nested functions so
            # cross-calls between nested functions (e.g. calc calling doubler) resolve correctly.
            nested_fns = getattr(decl, 'nested_functions', [])
            saved_nested_outer = dict(self._nested_name_map)
            ...
            for nf in nested_fns:
                ...
                self.lines = saved
                nf.name = saved_name
            self._nested_name_map = saved_nested_outer
            saved = self.lines
            self.lines = []
    ...
    def gen_type_decl(self, decl):
        # Check if any field references this type (self-referential struct)
        self_ref = any(
            f.type_.name == decl.name or
            (f.type_.pointer and f.type_.name == decl.name)
            for f in decl.fields
        )
        if self_ref:
            self.emit(f"typedef struct {decl.name} {decl.name};")
        if getattr(decl, 'packed', False):
            tag = f" {decl.name}" if self_ref else ""
            self.emit(f"typedef struct{tag} __attribute__((packed)) {{")
        else:
            tag = f" {decl.name}" if self_ref else ""
            self.emit(f"typedef struct{tag} {{")
        self.indent()
        if decl.parent:
            self.emit(f"{decl.parent} _base;")
        for f in decl.fields:
            if f.type_.is_array:
                base = UL_TO_C.get(f.type_.name, f.type_.name)
                if f.type_.array_size and f.type_.array_size > 0:
                    # Fixed-size array field: uint32_t state[4]
                    self.emit(f"{base} {f.name}[{f.type_.array_size}];")
                else:
                    # Unsized array — emit as pointer (runtime allocation needed)
                    self.emit(f"{base}* {f.name};")
            else:
    ...
    def gen_function(self, decl):
        ...
        # Track declared variables in python_compat mode for auto-declaration on first assign
        saved_declared = getattr(self, '_declared_vars', None)
        self._declared_vars = {p.name for p in decl.params}
        ...
        # Restore declared vars from enclosing scope
    ...
    def gen_stmt(self, node):
        ...
        if t == VarDecl:
            ...
            if hasattr(self, '_declared_vars'):
                self._declared_vars.add(node.name)
            # Function pointer: none <parson_malloc> = malloc(int size)
            if isinstance(node.init, FuncPtrValue) and node.type_.pointer:
                ret_ctype = UL_TO_C.get(node.type_.name, node.type_.name)
                if ret_ctype == "void":
                    ret_ctype = "void*"  # none pointer returns void*
                param_types = []
                for p in node.init.params:
                    pt = self.c_type(p.type_)
                    if p.type_.pointer:
                        pt += "*"
                    param_types.append(pt)
                params_str = ", ".join(param_types) if param_types else "void"
                func_name = node.init.function
                # Map UL builtins back to C names
                func_map = {"memtake": "malloc", "memgive": "free"}
                c_func = func_map.get(func_name, func_name)
                self.emit(f"{ret_ctype} (*{node.name})({params_str}) = {c_func};")
                return
            ...
            if node.type_.is_array and isinstance(node.init, ArrayLiteral) and len(node.init.elements) == 0:
                ...
                # When assigning int c = string[i], emit char code not char* pointer
                _was = getattr(self, '_string_index_as_int', False)
                if not node.type_.is_array and not node.type_.is_list and node.type_.name in ('int', 'float', 'double', 'bool') and isinstance(node.init, Index):
                    self._string_index_as_int = True
                val = self.gen_expr(node.init)
                self._string_index_as_int = _was
                # User-defined struct types cannot be initialized with a bare integer (e.g. 0).
                # Emit {0} aggregate initializer instead so gcc accepts it.
                if (node.type_.name in self.type_decls and val == "0"
                        and not node.type_.is_array and not node.type_.is_list):
                    self.emit(f"{ctype} {node.name} = {{0}};")
                else:
            ...
            # In python_compat mode, auto-declare variables on first assignment
            if (self._python_compat and node.op == "=" and isinstance(node.target, Identifier)
                    and hasattr(self, '_declared_vars')
                    and node.target.name not in self._declared_vars
                    and node.target.name not in self.var_types):
                val = self.gen_expr(node.value)
                self.emit(f"__auto_type {node.target.name} = {val};")
                self._declared_vars.add(node.target.name)
            elif self._refcount_mode and isinstance(node.target, Identifier):
                vtype = self.var_types.get(node.target.name)
                if vtype and vtype.name == "string":
                ...
                # When assigning int variable from string[i], emit char code not char* pointer
                _was = getattr(self, '_string_index_as_int', False)
                if isinstance(node.target, Identifier) and node.op == "=" and isinstance(node.value, Index):
                    lhs_type = self.var_types.get(node.target.name)
                    if lhs_type and lhs_type.name in ('int', 'float', 'double', 'bool') and not lhs_type.is_array and not lhs_type.is_list:
                        self._string_index_as_int = True
                # Array reassignment: dst = [0,0,...] → memset(dst, 0, sizeof(dst))
                if node.op == "=" and isinstance(node.value, ArrayLiteral):
                    lhs = self.gen_expr(node.target)
                    self._string_index_as_int = _was
                    all_zero = all(isinstance(e, IntLiteral) and e.value == 0 for e in node.value.elements)
                    if all_zero:
                        self.emit(f"memset({lhs}, 0, sizeof({lhs}));")
                    else:
                        elems = ", ".join(self.gen_expr(e) for e in node.value.elements)
                        n = len(node.value.elements)
                        elem_type = self.var_types.get(node.target.name) if isinstance(node.target, Identifier) else None
                        if elem_type and hasattr(elem_type, 'name'):
                            ct = UL_TO_C.get(elem_type.name, elem_type.name)
                            self.emit(f"memcpy({lhs}, ({ct}[]){{{elems}}}, {n} * sizeof({ct}));")
                        else:
                            self.emit(f"memcpy({lhs}, (int[]){{{elems}}}, {n} * sizeof(int));")
                else:
                    lhs = self.gen_expr(node.target)
                    rhs = self.gen_expr(node.value)
                    self._string_index_as_int = _was
                    # Struct assignment from bare 0 → memset to zero
                    if node.op == "=" and rhs == "0" and isinstance(node.target, Identifier):
                        lhs_type = self.var_types.get(node.target.name)
                        if (lhs_type and lhs_type.name in self.type_decls
                                and not lhs_type.is_array and not lhs_type.is_list):
                            self.emit(f"memset(&{lhs}, 0, sizeof({lhs}));")
                        else:
            ...
            # Bool: print "True"/"False" to match Python target output
            if self._is_bool_expr(node.value):
                self.emit(f'printf("%s\\n", ({val}) ? "True" : "False");')
            # Float: use __ul_print_float helper to match Python repr (no trailing zeros)
            elif fmt == "%f":
                self.emit(f'__ul_print_float((double)({val}));')
            else:
    ...
    def gen_expr(self, node):
        ...
        if t == BinaryOp:
            if node.op == "|>":
                ...
                return f"{right}({left})"
            # Helper: detect a "bare string[i]" node that should be an int in arithmetic context.
            # An Index node where the target is a string variable and the op is numeric.
            def _is_str_index_node(n):
                return isinstance(n, Index) and isinstance(n.target, Identifier) and \
                    (lambda v: v and hasattr(v, 'name') and v.name == 'string' and not v.is_array)(
                        self.var_types.get(n.target.name))
            # When: op is arithmetic/bitwise/comparison, one side is string[i], other is NOT string.
            # In this case the string[i] should emit a char code (int), not str_char_at (char*).
            _numeric_ops = ("+", "-", "*", "/", "%", "&", "|", "^", "<<", ">>",
                            "==", "!=", "<", ">", "<=", ">=", "and", "or")
            if node.op in _numeric_ops:
                left_is_str_idx = _is_str_index_node(node.left)
                right_is_str_idx = _is_str_index_node(node.right)
                left_is_str = self._is_string_expr(node.left) and not left_is_str_idx
                right_is_str = self._is_string_expr(node.right) and not right_is_str_idx
                # If NEITHER side is a non-index string expression but AT LEAST ONE is a string index,
                # then both sides are numeric (char code context).
                if (left_is_str_idx or right_is_str_idx) and not left_is_str and not right_is_str:
                    _was = getattr(self, '_string_index_as_int', False)
                    self._string_index_as_int = True
                    left = self.gen_expr(node.left)
                    right = self.gen_expr(node.right)
        ...
        if t == Cast:
            ...
            if isinstance(inner, Call) and inner.name == "change" and len(inner.args) == 1:
                inner = inner.args[0]
            # cast(0, none) → NULL (not (void)0)
            if node.target_type.name == "none" and isinstance(inner, IntLiteral) and inner.value == 0:
                return "NULL"
            ...
            if node.target_type.name == "string":
                ...
                return buf
            # cast(string[i], int) → (unsigned char)(s[i]) — get char code, not pointer
            if isinstance(inner, Index) and node.target_type.name not in ("string",):
                target_name = inner.target.name if isinstance(inner.target, Identifier) else None
                if target_name and self.var_types.get(target_name) and \
                   hasattr(self.var_types[target_name], 'name') and \
                   self.var_types[target_name].name == "string":
                    idx = self.gen_expr(inner.index)
                    return f"((unsigned char)({target_name}[{idx}]))"
            inner_expr = self.gen_expr(inner)
            # Casting to a struct/user type → always a pointer cast in C
            # (you can't cast to a struct value, only to a struct pointer)
            if node.target_type.name in self.type_decls:
                return f"(({ctype}*){inner_expr})"
            # If target type is char and inner is a pointer, emit char* cast
            if node.target_type.name == "char":
                # Check if inner is a pointer variable
                is_inner_ptr = False
                if isinstance(inner, Identifier):
                    ivtype = self.var_types.get(inner.name)
                    if ivtype and ivtype.pointer:
                        is_inner_ptr = True
                # Also pointer if it's arithmetic on a pointer
                if is_inner_ptr or node.target_type.pointer:
                    return f"((char*){inner_expr})"
            return f"(({ctype}){inner_expr})"
        ...
        if t == Call:
            ...
            # char_from_code(n) → single-char string from ASCII code
            if node.name == "char_from_code":
                n_expr = self.gen_expr(call_args[0])
                return f"(__ul_char_from_code({n_expr}))"
        ...
        if t == Index:
            ...
            if isinstance(node.target, Identifier):
                ...
                # String indexing: s[i] → direct C array access (string is char* underneath)
                if vtype and hasattr(vtype, 'name') and vtype.name == 'string' and not vtype.is_array:
                    index_expr = self.gen_expr(node.index)
                    return f"{node.target.name}[{index_expr}]"
        ...
        if t == MethodCall:
            ...
            if isinstance(node.target, Identifier):
                vtype = self.var_types.get(node.target.name)
                # Check is_list BEFORE checking vtype.name == "string" so that
                # list<string> is handled as a list, not as a plain string variable.
                if vtype and vtype.name == "string" and not vtype.is_list:
                ...
                if vtype and vtype.is_array:
                    ...
                    # .len() on a static array: return the size directly (no array_len function)
                    if node.method == "len":
                        return size
            ...
            # Check if target is a plain TypeDecl (struct), not an ObjectDecl.
            # Plain structs don't have vtables — emit as function pointer field call.
            target_type_name = None
            if isinstance(node.target, Identifier):
                vtype = self.var_types.get(node.target.name)
                if vtype:
                    target_type_name = vtype.name
            elif isinstance(node.target, FieldAccess):
                ftype = self._resolve_field_type(node.target)
                if ftype:
                    target_type_name = ftype.name
            if target_type_name and target_type_name in self.type_decls:
                td = self.type_decls[target_type_name]
                if not isinstance(td, ObjectDecl):
                    # Treat as function pointer field call on a plain struct
                    fn_args = args if args else ""
                    # Use -> for pointer variables, . for value variables
                    access_op = "."
                    if isinstance(node.target, Identifier):
                        vt = self.var_types.get(node.target.name)
                        if vt and vt.pointer:
                            access_op = "->"
                    return f"(({target}{access_op}{node.method})({fn_args}))"
        ...
        if t == FuncPtrValue:
            return node.function  # just the function name — C resolves it
    ...
    def _gen_field_access(self, node):
        ...
        # Detect pointer variable or pointer field → use -> instead of .
        access_op = "."
        if isinstance(node.target, Identifier):
            vtype = self.var_types.get(node.target.name)
            if vtype and vtype.pointer:
                access_op = "->"
        elif isinstance(node.target, FieldAccess):
            # Nested field access: check if the intermediate field is a pointer
            ftype = self._resolve_field_type(node.target)
            if ftype and ftype.pointer:
                access_op = "->"
        if target_type_name and target_type_name in self.type_decls:
            base_chain = self._base_chain(target_type_name, node.field)
            return f"{self.gen_expr(node.target)}{access_op}{base_chain}{node.field}"
        return f"{self.gen_expr(node.target)}{access_op}{node.field}"
    ...
    def _is_bool_expr(self, node):
        """Check if an expression is a boolean type (should print True/False)."""
        if isinstance(node, BoolLiteral):
            return True
        if isinstance(node, Identifier):
            vt = self.var_types.get(node.name)
            if vt and hasattr(vt, 'name') and vt.name == 'bool':
                return True
        if isinstance(node, Call):
            fn_decl = self._func_decls.get(node.name)
            if fn_decl and fn_decl.return_type and fn_decl.return_type.name == 'bool':
                return True
        if isinstance(node, FieldAccess):
            ftype = self._resolve_field_type(node)
            if ftype and ftype.name == 'bool':
                return True
        return False
    ...
    def _is_string_expr(self, node):
        ...
        if isinstance(node, Identifier):
            ...
            if vt and hasattr(vt, 'name') and vt.name == 'string':
                return True
            # Inside an object method, check if identifier is a string field of self
            if self._current_object and node.name in self._object_fields:
                obj_decl = self._current_object
                for f in self.all_fields(obj_decl.name):
                    if f.name == node.name and hasattr(f, 'type_') and f.type_.name == 'string':
                        return True
        if isinstance(node, FieldAccess):
            ftype = self._resolve_field_type(node)
            if ftype and hasattr(ftype, 'name') and ftype.name == 'string':
                return True
        if isinstance(node, BinaryOp) and node.op == '+':
        ...
        if isinstance(node, Call) and node.name in ('str_char_at', 'str_substr', 'reverse_string', 'caesar_cipher'):
            return True
        if isinstance(node, Cast) and hasattr(node, 'target_type') and node.target_type.name == 'string':
            return True
        return False
