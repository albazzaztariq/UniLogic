...
class TypeName:
    ...
    pointer: bool = False            # true if variable name wrapped in < > (e.g. int <p>)
    line: int = 0
    col: int = 0
...
@dataclass
class FuncPtrValue:
    function: str                    # the function name being pointed to (e.g. "malloc")
    params: List['Param']            # parameter types (e.g. [Param(int, "size")])
    line: int = 0; col: int = 0

@dataclass
class SymbolDecl:
    operator: str                    # the operator string (e.g. "+")
    function: str                    # function name to bind to
    line: int = 0; col: int = 0

@dataclass
class AliasDecl:
    name: str                        # alias name (e.g. UserId)
    target_type: 'TypeName'          # the type being aliased (e.g. int)
    line: int = 0; col: int = 0
...
class TypeDecl:
    ...
    packed: bool = False             # true if 'pack type' — no padding in C
    line: int = 0; col: int = 0
...
class MacroDecl:
    name: str
    params: List[str]                # macro parameter names
    expr: Any                        # body expression (AST node)
    line: int = 0; col: int = 0
