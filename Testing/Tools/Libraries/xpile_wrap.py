...
XPILE_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
VERIFY_DIR = os.path.join(XPILE_ROOT, "Testing", "VerifyTest")
...
def detect_compiler_and_command(origin_file, new_file, target):
    ...
    if target == "ul":
        if origin_ext == ".c":
            script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "c2ul.py")
            cmd = [PYTHON, "-B", script, origin_file, "-o", new_file]
            return script, cmd, "c2ul"
        elif origin_ext == ".py":
            script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "py2ul.py")
            cmd = [PYTHON, "-B", script, origin_file, "-o", new_file]
